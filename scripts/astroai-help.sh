...
astroai_cmd "  astroai-home-clean --all-safe   clear re-downloadable junk on /arc (\$HOME)"
astroai_cmd "  astroai-home-clean --hf     also drop Hugging Face models (expensive)"
astroai_cmd "  astroai-cache-prune --all-safe   clear scratch download caches (TMP_SCRATCH_DIR)"
...
astroai_cmd "  gh, rg, fd, fzf, bat, hyperfine, delta, tldr   search, browse, benchmark"
astroai_cmd "  sg (ast-grep)               astroai-install ast-grep  (syntax-aware search)"
...
astroai_heading "AI coding agents (once on /arc — see USAGE.md)"
astroai_cmd "  astroai-agent-setup             MCP + skills (new users: run first)"
astroai_cmd "  astroai-agent-setup update      refresh after image upgrade"
astroai_cmd "  astroai-agent-setup project     AGENTS.md in current repo"
astroai_cmd "  astroai-install agent           or claude, goose, opencode, codex, …"
astroai_cmd "  astroai-install --list          all agent CLIs"
astroai_hint "  then: gh auth login, pixi run / uv run, rg, fd"
echo ""
