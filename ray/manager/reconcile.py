...
def reconcile_cluster(
    *,
    ...
    # Batch Ray nodes fetch to avoid expensive repeated subprocess calls
    ray_nodes = list_ray_nodes()

    ray_ips = live_worker_node_ips(nodes=ray_nodes)
    head_ip = manager_pod_ip()
    worker_ray_ips = {ip for ip in ray_ips if ip != head_ip}
    ip_to_node = node_ip_to_id(nodes=ray_nodes)
